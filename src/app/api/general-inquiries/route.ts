...
export async function POST(request: Request) {
  console.log('✅ POST request received for contact inquiry');
  ...
  try {
    ...
    // Format the plain text email body
    let textBody = `📧 NEW CONTACT INQUIRY\n`;
    textBody += `${'='.repeat(60)}\n\n`;
    ...
    textBody += `Name:          ${data.fullName}\n`;
    textBody += `Email:         ${data.email}\n`;
    if (data.phone) textBody += `Phone:         ${data.phone}\n`;
    if (data.company) textBody += `Company:       ${data.company}\n`;
    textBody += `\n`;
    ...
    const htmlBody = `
      ...
      <head>
        ...
        <style>
          ...
          .info-table td:first-child { color: #666; font-weight: 600; width: 140px; }
          .info-table td:last-child { color: #333; }
          .message-block { background: #f9fafb; padding: 20px; border-left: 4px solid #9333ea; margin: 15px 0; line-height: 1.6; color: #374151; border-radius: 4px; white-space: pre-wrap; }
          .footer { padding: 30px; text-align: center; color: #9ca3af; font-size: 13px; border-top: 1px solid #e5e7eb; }
          a { color: #9333ea; text-decoration: none; }
          a:hover { text-decoration: underline; }
        </style>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="section">
              ...
              <div class="message-block">${data.message.replace(/\n/g, '<br>')}</div>
            </div>
          </div>
          ...
          <div class="footer">
            ...
            <p style="margin: 0; color: #d1d5db;">This email was sent from the MistrAI Contact Form</p>
          </div>
        </div>
      </body>
      </html>
    ...
    const mailOptions = {
      from: '"MistrAI Contact Form" <karanbhatt230803@gmail.com>',
      to: 'karanbhatt230803@gmail.com',
      replyTo: data.email,
      subject: `📧 Contact Inquiry from ${data.fullName}${data.company ? ` (${data.company})` : ''}`,
      text: textBody,
      html: htmlBody,
    };
